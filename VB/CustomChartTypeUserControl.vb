	...
	Partial Public Class CustomChartTypeUserControl
		...
		Public Sub New()
			...
			For Each imagePath As String In Directory.GetFiles(Application.StartupPath & "\..\..\Images")
				...
				Dim viewType As ViewType = CType(System.Enum.Parse(GetType(ViewType), Path.GetFileNameWithoutExtension(imagePath)), ViewType)
				imageListBoxControl1.Items.Add(viewType, imageCollection1.Images.Count - 1)
			Next imagePath
			imageListBoxControl1.ImageList = imageCollection1
		End Sub

		Private Sub CustomChartTypeUserControl_Load(ByVal sender As Object, ByVal e As EventArgs) Handles MyBase.Load
